...
- (void)loadView
{
    ...
    _container.backgroundColor			= [[UIColor alloc] initWithPatternImage:[UIImage imageNamed:@"bg.png"]];
    ...
    // setup caption
    _captionContainer.backgroundColor			= [UIColor blackColor];
    ...
    // setup thumbs view
    _thumbsView.backgroundColor					= [[UIColor alloc] initWithPatternImage:[UIImage imageNamed:@"bg.png"]];    _thumbsView.hidden							= YES;
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            UIBarButtonItem *btn = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Preglej vse", @"") style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)];
...
{
    if (!_hideTitle){
        [self setTitle:[NSString stringWithFormat:@"%i %@ %i", _currentIndex+1, NSLocalizedString(@"od", @"") , [_photoSource numberOfPhotosForPhotoGallery:self]]];
    }else{
        [self setTitle:@""];
    }
}
...
- (void)showThumbnailViewWithAnimation:(BOOL)animation
{
    ...
    [self arrangeThumbs];
    [self.navigationItem.rightBarButtonItem setTitle:NSLocalizedString(@"Zapri", @"")];
...
{
    _isThumbViewShowing = NO;
    [self.navigationItem.rightBarButtonItem setTitle:NSLocalizedString(@"Pokaži vse", @"")];
...
- (void)galleryPhoto:(FGalleryPhoto*)photo didLoadFullsize:(UIImage*)image
{
	...
	if( _currentIndex == photo.tag )
	{
        ...
        [photoView.imageView.layer setBorderColor: [[UIColor clearColor] CGColor]];
        [photoView.imageView.layer setBorderWidth: 2.0];
        photoView.imageView.layer.cornerRadius = 4;
        photoView.imageView.layer.masksToBounds = YES;
